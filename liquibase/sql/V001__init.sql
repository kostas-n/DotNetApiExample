...
CREATE TABLE example_surveys.response(
...
CREATE TABLE example_surveys.answer(
...
ALTER TABLE example_surveys.answer ADD CONSTRAINT fk_answer_response_id
	FOREIGN KEY (response_id) REFERENCES example_surveys.response(response_id);

-- Read models
CREATE TABLE example_surveys.latest_answer(
	...
	SELECT r.response_id, NVL(COUNT(a.response_id), 0) answers_count
	  ...
	  LEFT JOIN (
		SELECT la.response_id, la.changed_dtm, ROW_NUMBER() OVER (PARTITION BY la.response_id, la.question_id ORDER BY la.answer_id DESC) rn
		  FROM example_surveys.latest_answer la
...
-- todo: Fks
GRANT INSERT, UPDATE, DELETE, SELECT ON example_surveys.response TO easy_surveys_user;
GRANT INSERT, UPDATE, DELETE, SELECT ON example_surveys.answer TO easy_surveys_user;
GRANT INSERT, UPDATE, DELETE, SELECT ON example_surveys.latest_answer TO easy_surveys_user;
GRANT INSERT, UPDATE, DELETE, SELECT ON example_surveys.response_view TO easy_surveys_user;
